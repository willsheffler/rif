...
function download_boost(){
  echo $ME:$FUNCNAME: 'tools/build_utils.sh:download_boost(): BEGIN'
  ...
  if [[ $__VERSION ]]; then
    ...
    if [[ ! -f $BOOST_DIR/.is_downloaded ]]; then
      ...
      if [[ $__VERSION == "trunk" ]]; then
        ...
        $TRAVIS_RETRY git clone --depth 1 --recursive $__QUIET ${BOOST_URL} ${BOOST_DIR} || exit 1
...
function get_boost(){
  if [[ $1 ]]; then __DIR=$1; else echo $ME:$FUNCNAME: 'get_boost $1 must be location'; return; fi;
  if [[ $2 ]]; then __VERSION=$2; else __VERSION=1.58.0; fi

  if [[ $__VERSION ]]; then
    if [[ ! -f $BOOST_DIR/.is_built ]]; then
      download_boost $__DIR $__VERSION
      (cd ${BOOST_DIR} && ./bootstrap.sh && ./b2 -d0 --prefix=./ --shared --with-system --with-iostreams install && touch .is_built)
    else
...
function get_cmake(){
  ...
  if [[ ! -f $__DIR/cmake/.is_downloaded ]]; then
    ...
    mkdir -p $__DIR/cmake && $TRAVIS_RETRY wget --no-check-certificate $__QUIET -O - ${CMAKE_URL} | tar --strip-components=1 -xz -C $__DIR/cmake
...
function get_clang(){
  ...
  if [[ $__VERSION ]]; then
    ...
    if [[ ! -f $LLVM_DIR/.is_downloaded ]]; then
      ...
      $TRAVIS_RETRY wget $__QUIET -O - ${LLVM_URL}      | tar --strip-components=1 -xJ -C ${LLVM_DIR}
      echo $ME:$FUNCNAME: DOWNLOADING $LIBCXX_URL
      $TRAVIS_RETRY wget $__QUIET -O - ${LIBCXX_URL}    | tar --strip-components=1 -xJ -C ${LLVM_DIR}/projects/libcxx
      echo $ME:$FUNCNAME: DOWNLOADING $LIBCXXABI_URL
      $TRAVIS_RETRY wget $__QUIET -O - ${LIBCXXABI_URL} | tar --strip-components=1 -xJ -C ${LLVM_DIR}/projects/libcxxabi
      echo $ME:$FUNCNAME: DOWNLOADING $CLANG_URL
      $TRAVIS_RETRY wget $__QUIET -O - ${CLANG_URL}     | tar --strip-components=1 -xJ -C ${LLVM_DIR}/clang
...
function get_doxygen(){
  ...
  mkdir doxygen && $TRAVIS_RETRY wget $__QUIET -O - ${DOXYGEN_URL} | tar --strip-components=1 -xz -C doxygen
