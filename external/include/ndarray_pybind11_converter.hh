...
// https://github.com/asford/pybind_ndarray_example
...
class type_caster<ndarray::Array<Element, N, C>> {
 public:
  bool load(handle src, bool) {
    ...
    if (C > 0) {
      ...
      for (int i = 0; i < -C; ++i) {
        ...
        if (actualStride != requiredStride) {
          // PyErr_SetString(
          // PyExc_ValueError,
          //"numpy.ndarray does not have enough column-major contiguous
          //dimensions"
          //);
          return false;
        }
    ...
    value =
        external(const_cast<Element *>(buf.data()), shape, strides, src_ptr);

    return true;
  }
  static handle cast(const ndarray::Array<Element, N, C> &src,
    ...
    handle owner;
    if (src.getManager()) {
      owner = PyCapsule_New(new ndarray::Manager::Ptr(src.getManager()),
                            "ndarray.Manager", ndarray::detail::destroyCapsule);
    } else {
      owner = handle();
    }

    array_t<Element> result(std::move(shape), std::move(strides), src.getData(),
                            owner);
 ...
 public:
  static PYBIND11_DESCR name() {
    return type_descr(_<ndarray::Array<Element, N, C>>());
  }
  static handle cast(const ndarray::Array<Element, N, C> *src,
                     return_value_policy policy, handle parent) {
