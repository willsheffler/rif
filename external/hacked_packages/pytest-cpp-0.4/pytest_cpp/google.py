...
def _is_gtest_list_tests_header(line):
    stripped = line.strip()
    starts_main = stripped.startswith('int main')
    starts_slash = stripped.startswith('/')
    starts_dash = stripped.startswith('-')
    return starts_main or starts_slash or starts_dash
...
class GoogleTestFacade(object):
    ...
    def list_tests(self, executable):
        ...
        for line in output.splitlines():
            if _is_gtest_list_tests_header(line):
                continue
