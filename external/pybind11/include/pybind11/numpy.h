...
/* This will be true on all flat address space platforms and allows us to reduce
   the
   whole npy_intp / size_t / Py_intptr_t business down to just size_t for all
   size
...
struct numpy_internals {
  ...
  numpy_type_info *get_type_info(const std::type_info &tinfo,
    ...
    if (it != registered_dtypes.end()) return &(it->second);
    if (throw_if_missing)
...
inline numpy_internals &get_numpy_internals() {
  ...
  if (!ptr) load_numpy_internals(ptr);
  return *ptr;
}
...
template <typename T>
using is_pod_struct =
    all_of<std::is_pod<T>,  // since we're accessing directly in memory we need
                            // a POD type
...
size_t byte_offset_unsafe(const Strides &) {
  return 0;
}
...
class unchecked_reference {
 protected:
  ...
  unchecked_reference(const void *data, const size_t *shape,
                      const size_t *strides, enable_if_t<!Dyn, size_t>)
  ...
  template <bool Dyn = Dynamic>
  unchecked_reference(const void *data, const size_t *shape,
                      const size_t *strides, enable_if_t<Dyn, size_t> dims)
 ...
 public:
  ...
  const T &operator()(Ix... index) const {
    ...
    return *reinterpret_cast<const T *>(
        data_ + byte_offset_unsafe(strides_, size_t(index)...));
  }
  ...
  template <bool Dyn = Dynamic>
  enable_if_t<Dyn, size_t> size() const {
    return std::accumulate(shape_, shape_ + ndim(), (size_t)1,
                           std::multiplies<size_t>());
  }
...
class dtype : public object {
 public:
  ...
  static dtype from_args(object args) {
    ...
    if (!detail::npy_api::get().PyArray_DescrConverter_(args.release().ptr(),
                                                        &ptr) ||
        !ptr)
 ...
 private:
  ...
  dtype strip_padding(size_t itemsize) {
    ...
    for (auto field : attr("fields").attr("items")()) {
      ...
      if (!len(name) && format.kind() == 'V') continue;
...
class array : public buffer {
 public:
  ...
  array(const pybind11::dtype &dt, const std::vector<size_t> &shape,
    ...
    if (!tmp) pybind11_fail("NumPy: unable to create array!");
    if (ptr) {
      if (base) {
  ...
  template <typename T>
  array(const std::vector<size_t> &shape, const std::vector<size_t> &strides,
        const T *ptr, handle base = handle())
      : array(pybind11::dtype::of<T>(), shape, strides, (const void *)ptr,
              base) {}

  template <typename T>
  array(const std::vector<size_t> &shape, const T *ptr, handle base = handle())
  ...
  size_t size() const {
    return std::accumulate(shape(), shape() + ndim(), (size_t)1,
                           std::multiplies<size_t>());
  }

  /// Byte size of a single element
  size_t itemsize() const {
    return (size_t)detail::array_descriptor_proxy(
               detail::array_proxy(m_ptr)->descr)
        ->elsize;
  }
  ...
  const size_t *shape() const {
    return reinterpret_cast<const size_t *>(
        detail::array_proxy(m_ptr)->dimensions);
  }
  ...
  const size_t *strides() const {
    return reinterpret_cast<const size_t *>(
        detail::array_proxy(m_ptr)->strides);
  }
   ...
   * returned object,
   * and the caller must take care not to access invalid dimensions or dimension
   * indices.
   */
 ...
 protected:
  ...
  void check_writeable() const {
    if (!writeable()) throw std::domain_error("array is not writeable");
  }
  ...
  template <typename... Ix>
  void check_dimensions_impl(size_t axis, const size_t *shape, size_t i,
                             Ix... index) const {
    if (i >= *shape) {
 ...
 public:
  ...
  const T &at(Ix... index) const {
    ...
    return *(static_cast<const T *>(array::data()) +
             byte_offset(size_t(index)...) / itemsize());
  }
  ...
  /** Returns a proxy object that provides const access to the array's data
   * without bounds or
   * dimensionality checking.  Unlike `unchecked()`, this does not require that
   * the underlying
   * array have the `writable` flag.  Use with care: the array must not be
   * destroyed or reshaped
   * for the duration of the returned object, and the caller must take care not
   * to access invalid
...
struct pyobject_caster<array_t<T, ExtraFlags>> {
  ...
  bool load(handle src, bool convert) {
    if (!convert && !type::check_(src)) return false;
...
inline PYBIND11_NOINLINE void register_structured_dtype(
  ...
  for (auto &field : ordered_fields) {
    if (field.offset > offset) oss << (field.offset - offset) << 'x';
    // mark unaligned fields with '^' (unaligned native type)
    if (field.offset % field.alignment) oss << '^';
    oss << field.format << ':' << field.name << ':';
    offset = field.offset + field.size;
  }
  if (itemsize > offset) oss << (itemsize - offset) << 'x';
  oss << '}';
...
class multi_array_iterator {
 ...
 private:
  ...
  void increment_common_iterator(size_t dim) {
    for (auto &iter : m_common_iterator) iter.increment(dim);
  }
...
broadcast_trivial broadcast(const std::array<buffer_info, N> &buffers,
  ...
  ndim = std::accumulate(buffers.begin(), buffers.end(), size_t(0),
                         [](size_t res, const buffer_info &buf) {
...
struct vectorize_helper {
  ...
  object run(array_t<Args, array::forcecast> &... args,
             index_sequence<Index...> index) {
    ...
    if (trivial == broadcast_trivial::non_trivial) {
      ...
      for (size_t i = 0; i < size; ++i)
        output[i] = f((reinterpret_cast<Args *>(
            buffers[Index].ptr)[buffers[Index].size == 1 ? 0 : i])...);
    }
  ...
  void apply_broadcast(const std::array<buffer_info, N> &buffers,
    ...
    for (output_iterator iter = array_begin<Return>(output); iter != output_end;
         ++iter, ++input_iter) {
...
template <typename Func, typename Return, typename... Args>
detail::vectorize_helper<Func, Return, Args...> vectorize(const Func &f,
                                                          Return (*)(Args...)) {
...
auto vectorize(Func &&f)
    -> decltype(vectorize(std::forward<Func>(f), (FuncType *)nullptr)) {
