...
#include "kinematics/SceneIterator.hpp"
#include "types.hpp"
...
struct Conformation : util::meta::ContainerInstanceMap<ActorContainer> {
  typedef
      typename util::meta::ContainerInstanceMap<ActorContainer>::Keys Actors;
  Conformation() {}
...
template <class _Conformation,
          // class _Position,
          class Index>
struct BodyTplt {
  typedef BodyTplt<_Conformation, Index> This;
  typedef _Conformation Conformation;
  typedef _Conformation ConformationConst;  // TODO: fix this, requires figuring
                                            // out Cereal load_and_construct
  // typedef _Position Position;

  BodyTplt() : conformation_() {}
  BodyTplt(shared_ptr<ConformationConst> c) : conformation_(c) {}

  BodyTplt(This const &that) { conformation_ = that.conformation_; }
  ...
  bool operator==(This const &o) const {
    return *conformation_ == *o.conformation_;
  }
...
struct ActorAdder {
  ...
  ActorAdder(Scene &s, typename Scene::Index ib, boost::any const &a,
             bool &success)
...
struct ActorGetter {
  ...
  bool &success_;
  ActorGetter(Scene const &s, typename Scene::Index ib,
              typename Scene::Index ia, boost::any const &aex, boost::any &a,
              bool &success)
      : scene_(s),
        ib_(ib),
        ia_(ia),
...
struct ActorGetterNonconst {
  ...
  typename Scene::Index ib_, ia_;
  bool &success_;
  ActorGetterNonconst(Scene &s, typename Scene::Index ib,
                      typename Scene::Index ia, boost::any const &aex,
                      boost::any &a, bool &success)
      : scene_(s),
        ib_(ib),
        ia_(ia),
        actor_exemplar_(aex),
...
struct ActorClearer {
  ...
  typename Scene::Index ib_;
  bool &success_;
  ActorClearer(Scene &s, typename Scene::Index ib, boost::any const &aex,
               bool &success)
...
struct ActorCounter {
  ...
  typename Scene::Index ib_;
  int &count_;
  ActorCounter(Scene const &s, typename Scene::Index ib, boost::any const &a,
               int &count)
...
template <class _ActorContainers, class _Position, class _Index = uint64_t>
...
// }
  ...
  bool operator==(This const &o) const {
    assert(bodies_.size() == this->positions_.size());
    return this->bodies_ == o.bodies_ && this->symframes_ == o.symframes_ &&
  ...
  Conformation &mutable_conformation_asym(Index i) {
    return const_cast<Conformation &>(bodies_.at(i).conformation());
  }

  Conformation const &conformation(Index i) const {
    return bodies_.at(i % bodies_.size()).conformation();
  }
  ...
  Position const &__position_unsafe_asym__(Index i) const {
  ...
  Conformation const &__conformation_unsafe_asym__(Index i) const {
    return bodies_[i].conformation();
  }
  Conformation const &__conformation_unsafe__(Index i) const {
    return bodies_[i % bodies_.size()].conformation();
  }

  template <class Actor>
  typename boost::enable_if<impl::has_type_Position<Actor>, Actor>::type
  get_actor(Index ib, Index ia) const {
    return Actor(conformation(ib).template get<Actor>().at(ia),
                 this->position(ib));
  }
  template <class Actor>
  ...
  visit(Visitor &visitor) const {
    ...
    typedef typename f::result_of::value_at_key<Conformation, Actor1>::type
        Container1;
    typedef typename f::result_of::value_at_key<Conformation, Actor2>::type
        Container2;
    typedef util::container::ContainerInteractions<
        typename Scene::Position, Container1, Container2, Index>
        ContInter;
    ...
    for (Index i1 = 0; i1 < NBOD * NSYM; ++i1) {
      ...
      for (Index i2 = 0; i2 < NBOD * NSYM; ++i2) {
        ...
        if (i1 == i2 || (boost::is_same<Actor1, Actor2>::value && i2 <= i1))
          continue;
        ...
        Position const rel_pos =
            inverse(__position_unsafe__(i1)) * (__position_unsafe__(i2));
        ContInter::get_interaction_range(rel_pos, container1, container2,
                                         range);
        for (iter = get_cbegin(range), end = get_cend(range); iter != end;
             ++iter) {
  ...
  // TODO: fix this!
  template <class Interaction>
  struct has_interaction : m::bool_<true> {
  };  // f::result_of::has_key<MAP,Interaction>::value> {};

  template <class Interaction>
  struct interaction_placeholder_type {
    typedef typename impl::get_placeholder_type<Interaction, Index>::type type;
  };

  template <class Interaction>
  struct iter_type {
    typedef typename m::if_<
        util::meta::is_pair<Interaction>,
        typename m::if_<
  ...
  template <class Interaction>
  typename interactions_type<Interaction>::type get_interactions() const {
    typedef typename iter_type<Interaction>::type Iter;
    Iter beg = Iter::make_begin(*this);
    Iter end = Iter::make_end(*this);
    return std::make_pair(beg, end);
...
struct ActorDumpPDB {
  ...
  std::ostream &out;
  MetaData const &meta;
  ActorDumpPDB(Scene const &s, std::ostream &o, MetaData const &m)
