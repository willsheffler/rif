...
TEST(SceneObjective, basic) {
  typedef objective::ObjectiveFunction<
      m::vector<ScoreADI, ScoreADC, ScoreADIADI, ScoreADCADI>, Config>
      ObjFun;
...
TEST(SceneObjective, big_scene_performance) {
  ...
  std::cout << "This test performs 301.934M score calls, should take about a "
               "second when compiled with optimizations."
            << std::endl;
  ...
  if (false) performance_test_helper(scene, visitor);
  ScoreADIADI::ncalls = 0;
