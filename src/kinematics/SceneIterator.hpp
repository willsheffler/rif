...
struct Symmetric {
  ...
  static Conf const &conformation(Scene const &s, Index i) {
    return s.__conformation_unsafe__(i);
  }
  static Pos const &position(Scene const &s, Index i) {
    return s.__position_unsafe__(i);
  }
};
template <class Scene, class Actor>
struct NotSymmetric {
  ...
  static Conf const &conformation(Scene const &s, Index i) {
    return s.__conformation_unsafe_asym__(i);
  }
  static Pos const &position(Scene const &s, Index i) {
    return s.__position_unsafe_asym__(i);
  }
};
...
struct PlaceHolder {
  ...
  static Value deref(Scene const &, Index ib, Index ia) {
    return Value(ib, ia);
  }
};
template <class Scene, class Actor>
...
struct SceneIter1B
  ...
  typedef typename Scene::Index Index;
  SceneIter1B() {}
  SceneIter1B(Scene const &s, Index ib, Index ia)
      : scene_(&s), ibody_(ib), iactor_(ia) {}
  static THIS make_begin(Scene const &s) {
    return THIS(s, next_nonempty(s, (Index)0), (Index)0);
  }
  static THIS make_end(Scene const &s) {
    return THIS(s, Sym::nbodies(s), (Index)0);
  }
  static Index next_nonempty(Scene const &s, Index i) {
    // cout << "next_nonempty " << i;
    while (i < Sym::nbodies(s) &&
           0 == Sym::conformation(s, i).template get<Actor>().size())
      ++i;
    // cout << " -> " << i << endl;
    return i;
  }
 ...
 private:
  ...
  void increment() {
    ...
    ++iactor_;
    if (iactor_ ==
        Sym::conformation(*scene_, ibody_).template get<Actor>().size()) {
      iactor_ = 0;
      ibody_ = next_nonempty(*scene_, ++ibody_);
    }
  }
  typename Deref::Value const dereference() const {
...
struct CountPairNoDuplicates {
  ...
  static void next(Index &i, Index &j, size_t s0, size_t s1) {
    ++j;
...
struct CountPairUpperTriangle {
  template <class Index>
  static void next(Index &i, Index &j, size_t, size_t s1) {
    ++j;
    if (j >= s1) {
      ++i;
      j = i + 1;
...
struct SceneIter2B
    : boost::iterator_facade<
          SceneIter2B<_Scene, std::pair<typename _Interaction::first_type,
  ...
  typedef util::container::ContainerInteractions<typename Scene::Position,
                                                 Container1, Container2, Index>
      ContInter;
  typedef typename ContInter::Range ContRange;

  SceneIter2B() {}
  SceneIter2B(Scene const &s, Index ib1, Index ib2)
      : scene_(&s), ibody1_(ib1), ibody2_(ib2) {}
  void update_range() {  // X * A = B, X = B * ~A
    assert(ibody1_ < scene_->nbodies_asym() ||
           ibody2_ < scene_->nbodies_asym());
    ContInter::get_interaction_range(
        scene_->__position_unsafe__(ibody2_) *
            inverse(scene_->__position_unsafe__(ibody1_)),
    ...
    // cout << "UPDATE_RANGE " << ibody1_ << " " << ibody2_ << " beg " << *iter_
    // << " end " << *end_ << endl;
  }
  static THIS make_begin(Scene const &s) {
    ...
    if (i < (Index)s.nbodies() && j < (Index)s.nbodies() &&
        (i < (Index)s.nbodies_asym() || j < (Index)s.nbodies_asym()))
      beg.update_range();
    return beg;
  }
  static THIS make_end(Scene const &s) {
    ...
    return THIS(s, std::numeric_limits<Index>::max(),
                std::numeric_limits<Index>::max());
  }
  bool is_universal_end() const {
    return ibody1_ == std::numeric_limits<Index>::max() &&
           ibody2_ == std::numeric_limits<Index>::max();
  }
  static void next_while_empty(Scene const &s, Index &i, Index &j) {
    // cout << "next_while_empty " << i<<"-"<<j << endl;
    while (i < CountPair::max_body1(s.nbodies_asym(), s.nbodies()) &&
           j < s.nbodies() &&
 ...
 private:
  ...
  void increment() {
    ...
    if (iter_ == end_) {  // end_ of body/body inter, must update range_
      CountPair::next(ibody1_, ibody2_, scene_->nbodies_asym(),
                      scene_->nbodies());
      ...
      if (ibody1_ < scene_->nbodies() && ibody2_ < scene_->nbodies() &&
          (ibody1_ < scene_->nbodies_asym() ||
           ibody2_ < scene_->nbodies_asym()))
        update_range();
      // std::cout << "redo range_ " << ibody1_ << " " << ibody2_ << std::endl;
      // std::cout << "SceneIter2B NEW BODIES: " << ibody1_ << " " << ibody2_ <<
      // std::endl;
    }
    // not valid when at end
    // assert( ibody1_ < scene_->nbodies_asym() );
    // assert( ibody2_ < scene_->nbodies() );
    // assert( iter_->first  < scene_->body(ibody1_).conformation().template
    // get<Actor1>().size() );
    // assert( iter_->second < scene_->body(ibody2_).conformation().template
    // get<Actor2>().size() );
  }
  ...
  bool equal(THIS const &o) const {
    ...
    if (o.is_universal_end()) {
      ...
      return ((o.ibody1_ >= o.scene_->nbodies() ||
               o.ibody2_ >= o.scene_->nbodies()) ||
              (o.ibody1_ >= o.scene_->nbodies_asym() &&
               o.ibody2_ >= o.scene_->nbodies_asym()));
    } else {
      std::cerr << "SceneIter2B can only compare to end!" << std::endl;
      std::exit(-1);
    }
    // // std::cout << "EQUAL " << ibody1_ << " " << ibody2_ << " / " <<
    // o.ibody1_ << " " << o.ibody2_ << std::endl;
