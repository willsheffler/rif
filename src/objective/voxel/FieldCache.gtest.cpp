...
struct Ellipse3D : Field3D<double> {
  ...
  Ellipse3D(double a, double b, double c, double d, double e, double f)
      : c1(a), c2(b), c3(c), sd1(d), sd2(e), sd3(f) {}
  double operator()(double f, double g, double h) const {
    ++ncalls;
    return 10.0 + std::exp(-sqr((f - c1) / sd1) - sqr((g - c2) / sd2) -
                           sqr((h - c3) / sd3));
  }
  template <class F3>
...
double test_cache_vs_field(Cache const &cache, Field field, int nsamp = 10000) {
  ...
  for (int i = 0; i < nsamp; ++i) {
    F3 samp =
        F3(uniform(rng), uniform(rng), uniform(rng)) * (cache.ub_ - cache.lb_) +
        cache.lb_;
...
TEST(FieldCache, test_accuracy_random) {
  ...
  ASSERT_LE(test_cache_vs_field(FieldCache3D<double>(field, -10, 13, 1.6),
                                field, 100000),
            0.30);
  ASSERT_LE(test_cache_vs_field(FieldCache3D<double>(field, -10, 13, 0.8),
                                field, 100000),
            0.15);
  ASSERT_LE(test_cache_vs_field(FieldCache3D<double>(field, -10, 13, 0.4),
                                field, 100000),
            0.08);
  ASSERT_LE(test_cache_vs_field(FieldCache3D<double>(field, -10, 13, 0.2),
                                field, 100000),
            0.04);
}
...
TEST(FieldCache, test_file_cache) {
  ...
  std::cout << "NOTE: the bounds-mismatch warning below is expected:"
            << std::endl;
...
TEST(BoundingFieldCache, test_bounding_ellipse_max) {
  ...
  for (int i = 0; i < 10000; ++i) {
    F3 idx = F3(uniform(rng), uniform(rng), uniform(rng)) * (f1.ub_ - f1.lb_) +
             f1.lb_;
...
TEST(BoundingFieldCache, test_bounding_delta) {
  ...
  double sum = 0;
  for (size_t i = 0; i < f1.num_elements(); ++i) sum += f1.data()[i];
  ASSERT_EQ(sum, 1.0);
  ...
  for (int i = 0; i < 100000; ++i) {
    F3 idx =
        F3(uniform(rng), uniform(rng), uniform(rng)) * (bf1.ub_ - bf1.lb_) +
        bf1.lb_;
    // if( idx.norm() < spread-sqrt(3.0) && bf1[idx]!=1.0) cout << "FAIL1 " <<
    // idx << " " << idx.norm()-spread << endl;
    // if( idx.norm() > spread+sqrt(3.0) && bf1[idx]!=0.0) cout << "FAIL0 " <<
    // idx << " " << idx.norm()-spread << endl;
    // not sure if cell diagonal sqrt(3) is tight bound here... seems like it
    // should be sqrt(3)/2...
    if (idx.norm() < spread - sqrt(3.0)) ASSERT_EQ(bf1[idx], 1.0);
    if (idx.norm() > spread + sqrt(3.0))
      ASSERT_LE(bf1[idx],
                0.00000000001);  // can be "uninitialized" vals in corners
  }
}
}
}
