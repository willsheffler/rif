...
TEST(XformHashNeighbors, Quat_BCC7_Zorder_can_repr_identity) {
  ...
  for (int j = 0; j < 10000; ++j) {
    for (int i = 2; i < 10; ++i) {
      XformHash_Quat_BCC7_Zorder<Xform> xh(0.1 + runif(rng), i,
                                           runif(rng) * 400.0 + 2.0);
      ...
      // x.translation()[0] = 2.0*runif(rng)-1.0; // this doesn't work...
      // even/odd cells
...
TEST(XformHashNeighbors,
     DISABLED_Quat_BCC7_Zorder_quaternion_reflection_symmetry) {
  double eps_tol = 0.0000001;
  if (sizeof(Float) == 4) eps_tol = 0.0001;
  using namespace Eigen;
  typedef uint64_t Key;
  ...
  for (int iter1 = 0; iter1 < 100; ++iter1) {
    XformHash_Quat_BCC7_Zorder<Xform> xh(runif(rng) + 0.1,
                                         2 + (int)(runif(rng) * 60.0),
                                         runif(rng) * 400 + 10.0);
...
TEST(XformHashNeighbors, Quat_BCC7_Zorder_key_symmetry) {
  ...
  XformHash_Quat_BCC7_Zorder<Xform> xh(
      runif(rng) + 0.1, 2 + (int)(runif(rng) * 60.0), runif(rng) * 400 + 10.0);

  for (int j = 0; j < NSAMP; ++j) {
    XformHash_Quat_BCC7_Zorder<Xform> xh(runif(rng) + 0.1,
                                         2 + (int)(runif(rng) * 20.0),
                                         runif(rng) * 400 + 10.0);
    for (int i = 0; i < NSAMP; ++i) {
      Xform x;
...
TEST(XformHashNeighbors, Quat_BCC7_Zorder_check_ori_neighbors) {
  ...
  for (int iter1 = 0; iter1 < NSAMP; ++iter1) {
    XformHash_Quat_BCC7_Zorder<Xform> xh(Float(1.0),
    ...
    for (int i = 0; i < NSAMP; ++i) {
      ...
      for (int j = 0; j < 300 * NSAMP; ++j) {
        ...
        if (fail) {
          qfail += Eigen::Vector4d(w, x, y, z);
          ofail += o;
          fails.push_back(nk);
        } else {
          qpass += Eigen::Vector4d(w, x, y, z);
          opass += o;
          passes.push_back(nk);
        }
      }
...
TEST(XformHashNeighbors, Quat_BCC7_Zorder_check_general_neighbors) {
  ...
  for (int iter1 = 0; iter1 < NSAMP; ++iter1) {
    double cart_bound = 1.0 + runif(rng) * 2.0,
           ang_bound = 10.0 + runif(rng) * 20.0;
    // double cart_bound=2.0, ang_bound=20.0;
    double quat_bound = numeric::deg2quat(ang_bound);
    ...
    for (int iter2 = 0; iter2 < NSAMP; ++iter2) {
      ...
      Xform c = xh.get_center(key);
      // xh.print_key(key);
      // int ix = (int)((util::undilate<7>( key>>1 ) & 63) |
      // ((key>>57)&127)<<6);
      // int iy = (int)((util::undilate<7>( key>>2 ) & 63) |
      // ((key>>50)&127)<<6);
      // int iz = (int)((util::undilate<7>( key>>3 ) & 63) |
      // ((key>>43)&127)<<6);
      ...
      cout << "TEST ITER " << iter1 << " " << iter2 << " " << cart_bound << " "
           << ang_bound << " " << failfrac << endl;
