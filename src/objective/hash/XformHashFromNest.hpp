...
struct XformHashFromNest {
  ...
  struct apply {
    ...
    BOOST_STATIC_ASSERT_MSG(XformCheck::value,
                            "Nest::Value must be same as Xform");
    ...
    void init(Float cart_resl, Float ang_resl_deg, Float cart_bound) {
    ...
    Index approx_size(int resl = 0) const { return nest_.size(resl); }

    static std::string name() {
      return "XformHashFromNest< " + _Nest::pmap_name() + " >";
    }
  };
};
}
