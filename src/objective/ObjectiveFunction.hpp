...
#include "util/meta/intersect.hpp"
...
namespace traits {
// struct result_type { template<class T> struct apply { typedef typename
// T::Result type; }; };
struct interaction_type {
  template <class T>
  struct apply {
    typedef typename T::Interaction type;
  };
};
}
...
struct get_interaction_from_placeholder_impl {
  Interaction operator()(PlaceHolder const &placeholder,
                         InteractionSource const &) {
    return placeholder;
  }
};
template <class Interaction, class PlaceHolder, class InteractionSource>
struct get_interaction_from_placeholder_impl<Interaction, PlaceHolder,
                                             InteractionSource, false> {
  Interaction operator()(PlaceHolder const &placeholder,
                         InteractionSource const &source) {
    return source.template get_interaction_from_placeholder<Interaction>(
        placeholder);
  }
};
template <class Interaction, class PlaceHolder, class InteractionSource>
Interaction get_interaction_from_placeholder(PlaceHolder const &placeholder,
                                             InteractionSource const &source) {
  return get_interaction_from_placeholder_impl<
      Interaction, PlaceHolder, InteractionSource,
      boost::is_same<PlaceHolder, Interaction>::value>()(placeholder, source);
}
...
struct objective_interactions_equal {
  ...
  struct apply
      : m::bool_<boost::is_same<Interaction,
                                typename Objective::Interaction>::value> {};
};

///@brief helper class builds mpl::vector of Objectives with
/// Objective::Interaction == Interaction
...
template <class Result, class Objective, class Interaction, class Scratch,
          class Config>
void call_objective(Result &result, Objective const &objective,
                    Interaction const &interaction, Scratch &scratch,
                    Config const &config) {
  result = objective(interaction, scratch, config);
}
template <class Result, class Objective, class Interaction, class Config>
void call_objective(Result &result, Objective const &objective,
                    Interaction const &interaction, NullScratch &,
                    Config const &config) {
  result = objective(interaction, config);
}
template <class Result, class Objective, class Actor1, class Actor2,
          class Scratch, class Config>
void call_objective(Result &result, Objective const &objective,
                    Actor1 const &actor1, Actor2 const &actor2,
...
template <class Result, class Objective, class Actor1, class Actor2,
          class Config>
void call_objective(Result &result, Objective const &objective,
                    Actor1 const &actor1, Actor2 const &actor2, NullScratch &,
                    Config const &config) {
...
struct EvalObjective {
  ...
  Config const &config;
  double weight;
  EvalObjective(Interaction const &i, Results &r, Scratches &s, Config const &c,
                double w)
  ...
  void operator()(Objective const &objective) const {
    BOOST_STATIC_ASSERT((
        f::result_of::has_key<typename Results::FusionType, Objective>::value));
#ifdef DEBUG_IO
    std::cout << "    EvalObjective:     Objective " << Objective::name()
              << "( " << interaction << " )" << std::endl;
#endif
    typedef
        typename f::result_of::value_at_key<Results, Objective>::type Result;
    /// TODO: unpack args to allow for tuples of std::ref, would avoid extra
    /// copy
    Result result;  // = objective( interaction, config );
    call_objective(result, objective, interaction,
                   scratches.template get<Objective>(), config);
...
struct EvalObjectiveSplitPair {
  ...
  Config const &config;
  double weight;
  EvalObjectiveSplitPair(typename Interaction::first_type const &i,
                         typename Interaction::second_type const &j, Results &r,
                         Scratches &s, Config const &c, double w)
      : actor_1(i),
        actor_2(j),
        results(r),
        scratches(s),
  ...
  void operator()(Objective const &objective) const {
    BOOST_STATIC_ASSERT((
        f::result_of::has_key<typename Results::FusionType, Objective>::value));
#ifdef DEBUG_IO
    std::cout << "    EvalObjectiveSplitPair:     Objective "
              << Objective::name() << "( " << interaction << " )" << std::endl;
#endif
    typedef
        typename f::result_of::value_at_key<Results, Objective>::type Result;
    /// TODO: unpack args to allow for tuples of std::ref, would avoid extra
    ...
    call_objective(result, objective, actor_1, actor_2,
                   scratches.template get<Objective>(), config);
    results.template get<Objective>() += weight * result;
  }
};
...
typename boost::disable_if<
    typename get_DefinesInteractionWeight_false_<InteractionSource>::type,
    double>::type
...
// call pre( Result &, Scratch &, Config const & ) iff objective has HasPre &&
// Scratch is not NullScratch
...
eval_objective_pre(InteractionSource const &source, Objective const &objective,
                   typename Objective::Result &result, Scratch &scratch,
                   Config const &config) {
  objective.pre(source, result, scratch, config);
}
// call pre( Result &, Config const & ) iff objective has HasPre and Scratch is
// NullScratch
template <class InteractionSource, class Objective, class Config>
typename boost::enable_if<typename get_HasPre_false_<Objective>::type>::type
eval_objective_pre(InteractionSource const &source, Objective const &objective,
                   typename Objective::Result &result, NullScratch &scratch,
                   Config const &config) {
...
eval_objective_post(InteractionSource const &source, Objective const &objective,
                    typename Objective::Result &result, Scratch &scratch,
                    Config const &config) {
...
eval_objective_pre(InteractionSource const &source, Objective const &,
                   typename Objective::Result &, Scratch &, Config const &) {
...
eval_objective_post(InteractionSource const &, Objective const &,
                    typename Objective::Result &, Scratch &, Config const &) {
  ;  // appropriate post function no available, do nothing
}

template <class InteractionSource, class Interaction, class Results,
          class Scratches, class Config>
struct EvalObjectivePre {
  InteractionSource const &source_;
  Results &results;
  Scratches &scratches;
  Config const &config;
  EvalObjectivePre(InteractionSource const &src, Results &r, Scratches &s,
                   Config const &c)
      : source_(src), results(r), scratches(s), config(c) {}
  template <class Objective>
  void operator()(Objective const &objective) const {
    BOOST_STATIC_ASSERT((
        f::result_of::has_key<typename Results::FusionType, Objective>::value));
#ifdef DEBUG_IO
    std::cout << "    EvalObjectivePost:     Objective " << Objective::name()
              << std::endl;
#endif
    eval_objective_pre(source_, objective, results.template get<Objective>(),
                       scratches.template get<Objective>(), config);
  }
};
...
struct EvalObjectivePost {
  ...
  Scratches &scratches;
  Config const &config;
  EvalObjectivePost(InteractionSource const &src, Results &r, Scratches &s,
                    Config const &c)
      : source_(src), results(r), scratches(s), config(c) {}
  template <class Objective>
  void operator()(Objective const &objective) const {
    ...
    std::cout << "    EvalObjectivePost:     Objective " << Objective::name()
              << std::endl;
#endif
    eval_objective_post(source_, objective, results.template get<Objective>(),
                        scratches.template get<Objective>(), config);
  }
};

template <class InteractionSource, class ObjectiveMap, class Results,
          class Scratches, class Config>
struct EvalObjectivesPre {
  ...
  EvalObjectivesPre(InteractionSource const &s, ObjectiveMap const &f,
                    Results &r, Scratches &scratches, Config const &c)
      : source_(s),
        objective_map_(f),
        results_(r),
        scratches_(scratches),
        config_(c) {}
  template <class Interaction>
  void operator()(util::meta::type2type<Interaction>) const {
    ...
    f::for_each(
        ...
        EvalObjectivePre<InteractionSource, Interaction, Results, Scratches,
                         Config>(source_, results_, scratches_, config_));
  }
};
template <class InteractionSource, class ObjectiveMap, class Results,
          class Scratches, class Config>
struct EvalObjectivesPost {
  ...
  Scratches &scratches_;
  Config const &config_;
  EvalObjectivesPost(InteractionSource const &s, ObjectiveMap const &f,
                     Results &r, Scratches &scratches, Config const &c)
      : source_(s),
        objective_map_(f),
        results_(r),
  ...
  void operator()(util::meta::type2type<Interaction>) const {
    ...
    f::for_each(
        objective_map_.template get<Interaction>(),
        EvalObjectivePost<InteractionSource, Interaction, Results, Scratches,
                          Config>(source_, results_, scratches_, config_));
  }
};
...
template <class InteractionSource, class ObjectiveMap, class Results,
          class Scratches, class Config, class Enable = void>
struct EvalObjectives {};

// this EvalObjectives is for using iteration, in the case that the
// InteractionSource dosen't support visitors
template <class InteractionSource, class ObjectiveMap, class Results,
          class Scratches, class Config>
struct EvalObjectives<
    InteractionSource, ObjectiveMap, Results, Scratches, Config,
    typename boost::disable_if<
        typename get_UseVisitor_false_<InteractionSource>::type>::type> {
  ...
  EvalObjectives(InteractionSource const &p, ObjectiveMap const &f, Results &r,
                 Scratches &s, Config const &c)
      : interaction_source_(p),
        objective_map_(f),
        results_(r),
        scratches_(s),
        config_(c) {
  ...
  void operator()(util::meta::type2type<Interaction>) const {
    BOOST_STATIC_ASSERT(
        (InteractionSource::template has_interaction<Interaction>::value));
#ifdef DEBUG_IO
    std::cout << "    EvalObjectives Interaction: ";
    util::meta::PrintType(std::cout).operator()(Interaction());
#endif
    typedef typename InteractionSource::template interaction_placeholder_type<
    ...
    BOOST_FOREACH (
      ...
      double weight =
          get_interaction_weight(interaction_source_, interaction_placeholder);
      // std::cout << "        Interaction: " << interaction <<
      // " Result: " << typeid(results_.template get<Objective>()).name() <<
      // std::endl;
      Interaction const &interaction =
          get_interaction_from_placeholder<Interaction, Placeholder,
                                           InteractionSource>(
              interaction_placeholder, interaction_source_);
      f::for_each(objective_map_.template get<Interaction>(),
...
template <class _Interaction, class Objectives, class Results, class Scratches,
          class Config>
struct ObjectivesVisitor {
  ...
  typename boost::enable_if<util::meta::is_pair<I>, void>::type operator()(
      typename I::first_type const &a1, typename I::second_type const &a2,
      double weight = 1.0) {
    f::for_each(objectives_,
...
// this EvalObjectives is for using the Visitor method instead of iteration,
// faster but requires support in the InteractionSource object
template <class InteractionSource, class ObjectiveMap, class Results,
          class Scratches, class Config>
struct EvalObjectives<
    InteractionSource, ObjectiveMap, Results, Scratches, Config,
  ...
  void operator()(util::meta::type2type<Interaction>) const {
    ...
    util::meta::PrintType(std::cout).operator()(Interaction());
#endif
    typedef typename f::result_of::value_at_key<ObjectiveMap, Interaction>::type
        Objectives;
    Objectives const &objectives = objective_map_.template get<Interaction>();
    ObjectivesVisitor<Interaction, Objectives, Results, Scratches, Config>
        visitor(objectives, results_, scratches_, config_);
...
template <typename _Objectives, typename _Config>
struct ObjectiveFunction {
  ...
  typedef typename m::copy<
      typename               // copy back into mpl::vector;
  ...
  typedef util::meta::NumericInstanceMap<
      Objectives,
      impl::get_Result_double<m::_1>  // TODO: fix this to avoid weights storing
                                      // extra stuff???
      >
      Weights;
  ...
  ///@brief accessor for Objectives, may be used to configure or initialize
  /// Objective instances
  template <class Objective>
  Objective &get_objective() {
    BOOST_STATIC_ASSERT(
        (true));  // shold check that objective is actuall in Objectives...
  ...
  void operator()(InteractionSource const &source, Config const &config,
                  Results &results) const {
    // make sure we only operate on interactions contained in source
    typedef typename impl::get_InteractionTypes_void<InteractionSource>::type
  ...
  ///@return a NumericInstanceMap of ResultTypes defined by Objectives
  template <class InteractionSource>
  Results operator()(InteractionSource const &source,
  ...
  ///@return a NumericInstanceMap of ResultTypes defined by Objectives
  template <class InteractionSource>
  Results operator()(InteractionSource const &source) const {
