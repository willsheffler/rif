...
struct SceneOjbective {
  ...
  virtual float score_with_rotamers(SceneBase const &s,
                                    Rotamers &rots) const = 0;
  virtual void score_with_rotamers(SceneBase const &s, std::vector<float> &vec,
                                   Rotamers &rots) const = 0;
  virtual bool is_compatible(SceneBase const &s) const = 0;
  virtual bool provides_rotamers() const = 0;
  std::vector<float> scores(SceneBase const &s) const {
...
struct SceneObjectiveParametric
  ...
  typedef SceneOjbective<typename _Scene::Position, typename _Scene::Index>
      Super;
  ...
  virtual float score(SceneBase const &s) const {
    Scene const &scene = static_cast<Scene const &>(s);
    return objective(scene, config).sum();
  }
  virtual float score_with_rotamers(SceneBase const &s, Rotamers &rots) const {
    Scene const &scene = static_cast<Scene const &>(s);
  ...
  virtual bool is_compatible(SceneBase const &s) const {
    return nullptr != dynamic_cast<Scene const *>(&s);
  }

  virtual bool provides_rotamers() const {
