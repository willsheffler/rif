...
std::ostream& operator<<(std::ostream& out,
                         ObjectiveFunction<O, C> const& obj) {
  typedef ObjectiveFunction<O, C> OBJ;
  out << "ObjectiveFunction" << std::endl;
  out << "    Interactions:" << std::endl;
  m::for_each<typename OBJ::InteractionTypes>(
      util::meta::PrintType(out, "        "));
  out << "    Objectives:" << std::endl;
  f::for_each((typename OBJ::ObjectiveMap::FusionType&)obj.objective_map_,
              util::meta::PrintBFMapofVec(out, "        "));
  out << "    RAW:" << std::endl;
