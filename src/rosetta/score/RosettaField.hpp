...
#include "rosetta/score/AnalyticEvaluation.hpp"
...
struct RosettaField {
  ...
  void init_atom_bins() {
    ...
    atom_bins_dim_[0] = std::max(
        1, (int)std::ceil((atom_bins_ub_[0] - atom_bins_lb_[0]) / bin_witdh_));
    atom_bins_dim_[1] = std::max(
        1, (int)std::ceil((atom_bins_ub_[1] - atom_bins_lb_[1]) / bin_witdh_));
