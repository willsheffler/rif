...
template <int DIM, class Value = util::SimpleArray<DIM, double>,
          class Index = size_t, class Float = double>
struct QuaternionMap {
  ...
  bool params_to_value(Params const &params, Index /*cell_index*/, Index resl,
    ...
    for (size_t i = 0; i < DIM; ++i)
      get_quaternion_component(value, i) = params[i];
    ...
    // std::cout << "corners " << lower_corner << " , " << upper_corner <<
    // std::endl;
  ...
  void value_to_params_for_cell(Value const &value, Index /*resl*/,
    ...
    for (size_t i = 0; i < DIM; ++i)
      assert(-1.0 <= get_quaternion_component(value, i));
    for (size_t i = 0; i < DIM; ++i)
      assert(1.0 >= get_quaternion_component(value, i));
    for (size_t i = 0; i < DIM; ++i)
  ...
  void get_neighboring_cells(Value const &value, Index /*resl*/,
    ...
    // int lb = std::max(                0, static_cast<int>(
    // value[0]-param_delta + BIG ) - BIG );
    // int ub = std::min((int)num_cells_-1, static_cast<int>(
    // value[0]+param_delta + BIG ) - BIG );
