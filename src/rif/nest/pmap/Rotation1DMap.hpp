#ifndef INCLUDED_nest_maps_Rotation1DMap_HH
#define INCLUDED_nest_maps_Rotation1DMap_HH
...
template <int DIM = 1, class Value = Eigen::Matrix3d, class Index = uint64_t,
          class Float = double>
struct Rotation1DMap {
  ...
  Rotation1DMap()
      : nside(1), lb(-M_PI), ub(M_PI), axis(0, 0, 1), flipaxis(0, 0, 0) {}
  Rotation1DMap(Params p, Params q, Indices i)
      : lb(p[0]), ub(q[0]), nside(i[0]), axis(0, 0, 1), flipaxis(0, 0, 0) {}
  ...
  bool params_to_value(Params const &params, Index cell_index, Index resl,
    ...
    // std::cout << "SET " << doflip <<  std::endl;//axis.transpose() << " " <<
    // ang*180/M_PI << std::endl;
  ...
  void get_axis_angle_and_flip(Value value, Eigen::Vector3d &ax, Float &ang,
    ...
    if (fabs(ang) > 0.00001 && ax.dot(axis) < 0.999) {
      ...
      // std::cout << ax.transpose() << " / " << axis.transpose() << " / " <<
      // ang*180/M_PI << std::endl;
      // std::cout << value << std::endl;
    }
  }
  ...
  bool value_to_params(Value const &value0, Index /*resl*/, Params &params,
    ...
    if (ang < -M_PI) ang += 2.0 * M_PI;
    // std::cout << lb << " " << ang << " " << ub << " /  " << nside << " " <<
    // aa.axis().transpose() << " " << aa.angle() << std::endl;
    BOOST_ASSERT(lb <= ang && ang <= ub);
