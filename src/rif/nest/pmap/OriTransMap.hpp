...
struct OriTransMap {
  ...
  BOOST_STATIC_ASSERT_MSG(sizeof(Value) == 48 || sizeof(Value) == 96,
                          "Value must be AffineCompact repr");
  ...
  Index num_cells() const {
    return ori_map_.num_cells() * trans_map_.num_cells();
  }

  static std::string pmap_name() {
    return "OriTransMap< " + OriMap::pmap_name() + ", " +
           TransMap::pmap_name() + " >";
  }
};
