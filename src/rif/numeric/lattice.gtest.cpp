...
F test_bcc_performance(size_t NSAMP, S const Nside, F const Width) {
  ...
  for (int i = 0; i < NSAMP; ++i) indices[i] = bcc[samples[i]];
  cout << "BCC DIM " << N
       << " lookup rate: " << (double)NSAMP / lookup_time.elapsed()
       << " sec / ";

  std::vector<V> centers(NSAMP);
  util::Timer getval_time;
  for (int i = 0; i < NSAMP; ++i) centers[i] = bcc[indices[i]];
  cout << " getval rate: " << (double)NSAMP / getval_time.elapsed() << " sec"
       << endl;
  ...
  F improvement = 1.0;
  for (int i = 0; i < N; ++i) improvement *= frac;
  cout << "     improvement over cubic: " << improvement / 2.0
       << " cov: " << maxdiff / Width * Nside << " vs. " << sqrt(N) / 2.0
       << endl;  // 2 x num samp as cubic
...
TEST(lattice, performance) {
  ...
  double Width = 10.0;
  double const R3test =
      test_bcc_performance<3, double, uint64_t>(NITER, Nside, Width);
  ...
  double const R3 = std::pow(2.0, -5.0 / 3.0) * sqrt(5) /
                    std::pow(2.0, 1.0 / 3.0) * Width / Nside;
  ASSERT_LE(R3test, R3);
...
F test_bcc_neighbors(size_t NSAMP) {
  ...
  for (F radius = 0; radius < 3.0 * inrad; radius += inrad / 10.0) {
    ...
    for (int i = 0; i < NSAMP; ++i) {
      ...
      V cen;
      for (int j = 0; j < N; ++j) cen[j] = (runif(rng) - 0.5) * 2.0 * inrad;
      if (bcc[cen] != i0) {
        --i;
        continue;
      }
    ...
    for (int i = 0; i < NSAMP; ++i) {
      // pick random point in i0 cell
      V cen;
      for (int j = 0; j < N; ++j) cen[j] = (runif(rng) - 0.5);
      ...
      // if( index == i0 ){ --i; continue; }
      if (std::find(nbrs_cubic.begin(), nbrs_cubic.end(), index) !=
          nbrs_cubic.end()) {
        ++sum_in_nbrs_cubic;
      }
    }
    printf("%i %7.3f %9.7f %9.7f %9.7f\n", N, radius / inrad,
           (F)sum_in_nbrs / NSAMP, (F)sum_in_nbrs_we / NSAMP,
...
F test_bcc_children(size_t NSAMP) {
  ...
  for (int i = 0; i < NSAMP; ++i) {
    ...
    V samp;
    for (int j = 0; j < N; ++j) samp[j] = (runif(rng) - 0.5) * 4.0 * inrad;
    if (bcc_parent[samp] != i0) {
      --i;
      continue;
    }
  ...
  printf("BCC child coverage: %i %3i %9.7f   %3i %9.7f\n", N, nbccFC,
         (F)sum_in_nbrs / NSAMP, nbccFCE, (F)sum_in_nbrs_we / NSAMP);
  return 0;
}
