...
struct XformHashNeighborCrappyIterator
    : boost::iterator_facade<XformHashNeighborCrappyIterator<XformHash, UNIQUE>,
...
struct XformHashNeighbors {
  ...
  XformHashNeighbors(Float cart_bound,
                     Float ang_bound,  // degrees
                     XformHash xh, double xcov_target = 100.0)
      : hasher_(xh) {
    ...
    nsamp_ = ang_nside * ang_nside * ang_nside * 3.0 *
             xcov_target;  // ~100 fold coverage
    fill_cart_shifts();
    // std::cout << "XformHashNeighbors aw=" << hasher_.ang_width() << ",
    // quat_bound=" << quat_bound_
    // << " ang_nside=" << ang_nside << " nsamp=" << nsamp_ << " cart_shifts="
    // << cart_shifts_.size() << std:: endl;
  }
    ...
    for (int16_t i = -di; i <= di; ++i) {
      for (int16_t j = -di; j <= di; ++j) {
        for (int16_t k = -di; k <= di; ++k) {
          if (Eigen::Vector3d(i - 0.5, j - 0.5, k - 0.5).norm() *
                      hasher_.cart_width() <
                  thresh ||
              Eigen::Vector3d(i, j, k).norm() * hasher_.cart_width() < thresh ||
              Eigen::Vector3d(i + 0.5, j + 0.5, k + 0.5).norm() *
                      hasher_.cart_width() <
                  thresh ||
              false) {
  ...
  std::vector<Key> const &get_ori_neighbors(Key key) {
    ...
    if (ori_cache_.find(ori_key) == ori_cache_.end()) {
      ...
      if (true) {
        // std::cout << "get_asym nbrs the hard way, store in " << ori_key <<
        // std::endl;
        ...
        {
          ori_cache_.insert(
              std::make_pair(ori_key, std::vector<Key>(keys.size())));
        ...
        std::cout << "FLIP: " << ((ksym >> 2) & 1) << " " << ((ksym >> 1) & 1)
                  << " " << ((ksym >> 0) & 1) << std::endl;
        for (int i = 0; i < asym_nbrs.size(); ++i) {
          Xform x = hasher_.get_center(ori_cache_[ori_key][i]);
          ...
          // std::cout <<
          // Eigen::Quaterniond(hasher_.get_center(ori_cache_[ori_key][i]).rotation()).coeffs().transpose()
          // << std::endl
          //           << Eigen::Quaterniond(hasher_.get_center(
          //           asym_nbrs[i]).rotation()).coeffs().transpose() <<
          //           std::endl;
        }
        out.close();
      }
  ...
  bool load(std::istream &in) {
    ...
    if (hasher_.name() != std::string(buf)) {
      std::cerr << "XformHashNeighbors::load, hasher type mismatch, expected "
