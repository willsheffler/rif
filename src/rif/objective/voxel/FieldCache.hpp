  ...
  FieldCache3D(Field3D<Float> const &field, F1 const &lb, F2 const &ub,
    ...
    if (io::read_cache(cache_loc_, *this)) {
      ...
      for (size_t i = 0; i < BASE::DIM; ++i)
        extents_eq &= extents[i] == this->shape()[i];
      if (Float3(lb) == this->lb_ && Float3(ub) == this->ub_ &&
          Float3(cs) == this->cs_ && extents_eq) {
        std::cout << "EXTENTS EQ, USING CACHE" << std::endl;
        check_against_field(field);
        return;
      }
      std::cout << "Warning, FieldCache3D bad cache " << cache_loc_
                << " bounds mismatch, recomputing..." << std::endl;
      this->resize(extents);
    }  // else {
#endif
    ...
    for (int o = 0; o < oversample; ++o) {
      for (int p = 0; p < oversample; ++p) {
        for (int q = 0; q < oversample; ++q) {
          ...
          // std::cout << ox << " " << oy << " " << oz << " " << cen[0] << " "
          // << cen[1] << " " << cen[2] << std::endl;
        ...
        for (int j = 0; j < this->shape()[1]; j += 8) {
          for (int k = 0; k < this->shape()[2]; k += 8) {
            ...
            if (fabs(test1 - test2) > tolerance) {
              std::cout << "FIELD MISMATCH stored: " << test1
                        << " recalculated: " << test2 << std::endl;
              ++nerror;
...
struct AggMin {
  ...
  static Float aggregate(Float agg, Float newval) {
    return std::min(agg, newval);
  }
};
template <class Float>
struct AggMax {
                                                      ...
                                                      // cache_loc_(cache_loc)
  {
    ...
    if (io::read_cache(cache_loc, *this)) {
      ...
      if (Float3(lb) == this->lb_ && Float3(ub) == this->ub_ &&
          Float3(cs) == this->cs_ && extents_eq)
        return;
      std::cout << "Warning, BoundingFieldCache3D bad cache " << cache_loc
                << " bounds mismatch, recomputing..." << std::endl;
      this->resize(extents);
    }
#endif
    if (!no_init) {
      // size_t ncalls = 0;
      for (Float h = this->lb_[2] + this->cs_[2] / 2.0;
           h < this->ub_[2] + this->cs_[2] / 2.0; h += this->cs_[2]) {
        for (Float g = this->lb_[1] + this->cs_[1] / 2.0;
             g < this->ub_[1] + this->cs_[1] / 2.0; g += this->cs_[1]) {
          for (Float f = this->lb_[0] + this->cs_[0] / 2.0;
               f < this->ub_[0] + this->cs_[0] / 2.0; f += this->cs_[0]) {
            // ++ncalls;
            this->operator[](Float3(f, g, h)) =
                calc_agg_val(ref, spread, Float3(f, g, h));
          }
        }
      }
    }
