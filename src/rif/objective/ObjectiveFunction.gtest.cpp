...
#include "objective/ObjectiveFunction_io.hpp"
...
struct SimpleInteractionSource {
  ...
  template <class Interaction>
  struct has_interaction
      : mpl::bool_<f::result_of::has_key<MAP, Interaction>::value> {};

  template <class Interaction>
  struct interaction_placeholder_type {
    typedef Interaction type;
  };
};
template <class I>
...
TEST(ObjectiveFunction, basic_tests_local_and_global_config) {
  typedef ObjectiveFunction<
      mpl::list<ScoreInt, ScoreInt2, ScoreDouble, ScoreIntDouble>, ConfigTest>
      ObjFun;
  ObjFun score;
...
TEST(ObjectiveFunction, test_results) {
  typedef ObjectiveFunction<mpl::vector<ScoreDouble, ScoreInt>, ConfigTest>
      ObjFun;
  ObjFun score;
...
struct PlaceholderInteractionSource {
  ...
  void add_interaction(Interaction const &interaction) {
    ...
    // cout << "add_interaction: " << interaction << " placeholder: " <<
    // vec.size()-1 << endl;
  }
...
TEST(ObjectiveFunction, test_iteraction_placeholder) {
  typedef ObjectiveFunction<
      mpl::list<ScoreInt, ScoreInt2, ScoreInt3, ScoreDouble, ScoreIntDouble>,
      ConfigTest>
      ObjFun;
  ObjFun score;
...
struct vec_of_rem_refwrap {
  template <class T>
  struct apply {
    typedef std::vector<typename util::meta::recursive_remove_refwrap<T>::type>
        type;
  };
};
...
struct RefInteractionSource {
  ...
  template <class Interaction>
  struct has_interaction
      : mpl::bool_<f::result_of::has_key<MAPREF, Interaction>::value> {};
...
TEST(ObjectiveFunction, tuple_of_ref_interactions) {
  typedef ObjectiveFunction<mpl::list<ScoreIntRefDoubleRef>, ConfigTest> ObjFun;
  ObjFun score;
  ...
  interaction_source.get_interactions_vals<I1>().push_back(
      std::make_pair(1, 1.5));
  interaction_source.store_refs();
  // std::vector< std::pair<int,double> > & tmp =
  // interaction_source.get_interactions_vals<I1>();
...
TEST(ObjectiveFunction, test_pre) {
  typedef ObjectiveFunction<mpl::list<ScoreInt>, ConfigTest> ObjFunNoPre;
  ObjFunNoPre scorenopre;

  typedef ObjectiveFunction<mpl::list<ScoreIntWithPre>, ConfigTest>
      ObjFunWithPre;
...
TEST(ObjectiveFunction, test_post) {
  typedef ObjectiveFunction<mpl::list<ScoreInt>, ConfigTest> ObjFunNoPost;
  ObjFunNoPost scorenopost;

  typedef ObjectiveFunction<mpl::list<ScoreIntWithPost>, ConfigTest>
      ObjFunWithPost;
...
TEST(ObjectiveFunction, test_scratch) {
  typedef ObjectiveFunction<mpl::list<ScoreInt>, ConfigTest> ObjFunNoScratch;
  ObjFunNoScratch scorenoscratch;

  typedef ObjectiveFunction<mpl::list<ScoreIntWithScratch>, ConfigTest>
      ObjFunWithScratch;
