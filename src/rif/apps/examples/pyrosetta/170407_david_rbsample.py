...
gn = gly
gn.append_pose_by_jump(asn, 1, 'CA', 'CG')  # anchor to 1st residue, rot round CG of res 2
j = gn.jump(1)  # only one jump in this system
j.set_translation(j.get_translation() + numeric.xyzVector_double_t(5, 0, 0))
gn.set_jump(1, j)  # move apart a bit

for i in range(10):
    j = gn.jump(1)
    print(j.gaussian_move(1, 0.0, 5.0))  # this is better than below, probably
