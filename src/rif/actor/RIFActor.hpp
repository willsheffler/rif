...
struct RIFActor {
  ...
  RIFActor(Position const &p, shared_ptr<XformMap> xmap)
      : position_(p), xform_map_(xmap) {}

  RIFActor(RIFActor const &actor0, Position const &moveby) {
  ...
  bool operator==(THIS const &o) const {
    return o.position_ == position_ && o.xform_map_ == xform_map_;
  }

  ///@brief necessary for testing only
  bool operator<(THIS const &o) const {
    return std::make_pair(position_, xform_map_) <
           std::make_pair(o.position_, o.xform_map_);
  }
...
std::ostream &operator<<(std::ostream &out, RIFActor<X, M> const &si) {
  return out << "RIFActor";
}
}
}
