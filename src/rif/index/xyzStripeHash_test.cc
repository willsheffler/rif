...
int slow_nbcount(core::pose::xyzStripeHashPose const &xyzhash, float DIST,
                 Vec const &rv) {
  ...
  for (int j = 0; j < (int)xyzhash.natom(); ++j) {
    ...
    if ((hx - tmp.x()) * (hx - tmp.x()) + (hy - tmp.y()) * (hy - tmp.y()) +
            (hz - tmp.z()) * (hz - tmp.z()) <=
        DIST * DIST)
      safe_nbcount++;
  }
...
void slow_nbset(core::pose::Pose const &pose, float DIST, Vec const &rv,
                std::set<int> &nbrs) {
  for (Size ir = 1; ir <= pose.n_residue(); ++ir) {
...
utility::vector1<Vec> slow_nbget(core::pose::xyzStripeHashPose const &xyzhash,
                                 float DIST, Vec const &rv) {
...
void dump_points_pdb(utility::vector1<Vec> &p, std::string fn) {
  ...
  for (Size i = 1; i <= p.size(); ++i) {
    std::string rn = "VIZ";
    o << "HETATM" << I(5, i) << ' ' << " CA " << ' ' << rn << ' ' << "A"
      << I(4, i) << "    " << F(8, 3, p[i].x()) << F(8, 3, p[i].y())
      << F(8, 3, p[i].z()) << F(6, 2, 1.0) << F(6, 2, 1.0) << '\n';
  }
  o.close();
}
...
struct CollectNeighbors {
  utility::vector1<Vec> nbrs;
  void visit(Vec const &, Vec const &c, float const &) { nbrs.push_back(c); }
};
...
void test_find_neighbors(core::pose::Pose const &p) {
  ...
  std::cout << "test find_neighbors " << to << " " << tg << " " << ts
            << std::endl;
}
...
int main(int argc, char *argv[]) {
  try {
    ...
    std::cout << "sizeof(utility::vector1< xyzVector<float> >) "
              ...
              << sizeof(xyzStripeHash::ushort2) << std::endl;
    ...
    core::import_pose::pose_from_pdb(
        p,
        basic::options::option[basic::options::OptionKeys::in::file::s]()[1]);
    if (true) {
      for (Size ir = 1; ir <= p.n_residue(); ++ir) {
        if (p.residue(ir).is_lower_terminus())
          core::pose::remove_lower_terminus_type_from_pose_residue(p, ir);
        if (p.residue(ir).is_upper_terminus())
          core::pose::remove_upper_terminus_type_from_pose_residue(p, ir);
      }
    }
    ...
    // for(xyzStripeHash<float>::xyz_iterator i = xyzhash.xyz_begin(); i !=
    // xyzhash.xyz_end(); ++i){
    //  numeric::xyzVector<float> const & a = *i;
    //  xyzStripeHash<float>::float4 const & b = *(j++);
    //  std::cout << a.x() <<" "<< b.x <<" "<< a.y() <<" "<< b.y <<" "<< a.z()
    // <<" "<< b.z << std::endl;
    ...
    if (basic::options::option[basic::options::OptionKeys::dump_hash]()) {
      ...
      std::cout << xyzhash.xdim() << " " << xyzhash.ydim() << " "
                << xyzhash.zdim() << std::endl;
      for (int j = 0; j < (int)xyzhash.natom(); ++j) {
      ...
      for (int j = 0; j < (int)xyzhash.xdim() * xyzhash.ydim() * xyzhash.zdim();
           ++j) {
    ...
    //      p.set_xyz(AtomID(ia,ir),p.xyz(AtomID(ia,ir)) +
    // (numeric::xyzVector<core::Real>)xyzhash.translation() );
    //  }
    // }
    ...
    std::cout << "ncells: " << xyzhash.xdim() * xyzhash.ydim() * xyzhash.zdim()
              << std::endl;
    ...
    for (int i = 0; i < (int)xyzhash.natom(); ++i) {
      hashpts.push_back(Vec(xyzhash.grid_atoms()[i].x(),
                            xyzhash.grid_atoms()[i].y(),
                            xyzhash.grid_atoms()[i].z()));
    }
    ...
    for (int iter = 0;
         iter <
         (int)
             basic::options::option[basic::options::OptionKeys::out::nstruct]();
         ++iter) {
      Vec rv(numeric::random::uniform(), numeric::random::uniform(),
             numeric::random::uniform());
      ...
      // }
      // for(utility::vector1<std::pair<int,int> >::const_iterator i =
      // hpairs.begin(); i != hpairs.end(); ++i){
      //    if(resset.count(i->second)!= 1) utility_exit_with_message("BAD
      // nbrs");
      // }
      ...
      tot += hash_nbcount;
      if (safe_nbcount >= 0 &&
          safe_nbcount != hash_nbcount /*|| safe_nbcount != ifc_nbcount*/) {
        std::cout << rv << std::endl;
        std::cout << safe_nbcount << " " << hash_nbcount << " " << ifc_nbcount
                  << std::endl;
        ...
        for (utility::vector1<Vec>::const_iterator i = tst.begin();
             i != tst.end(); ++i) {
          bool i_in = false;
          for (utility::vector1<Vec>::const_iterator j = ref.begin();
               j != ref.end(); ++j) {
            if (i->distance_squared(*j) < 0.0001) i_in = true;
          }
          if (!i_in)
            std::cout << "TST NOT IN REF: "
                      << ObjexxFCL::format::F(
                             20, 18, i->distance(rv + xyzhash.translation()))
                      << " " << (*i) << std::endl;
        }
        for (utility::vector1<Vec>::const_iterator i = ref.begin();
             i != ref.end(); ++i) {
          bool i_in = false;
          for (utility::vector1<Vec>::const_iterator j = tst.begin();
               j != tst.end(); ++j) {
            if (i->distance_squared(*j) < 0.0001) i_in = true;
          }
          if (!i_in)
            std::cout << "REF NOT IN TST: "
                      << ObjexxFCL::format::F(
                             20, 18, i->distance(rv + xyzhash.translation()))
                      << " " << (*i) << std::endl;
        }

        utility_exit_with_message("FAIL after " +
                                  ObjexxFCL::format::I(10, iter) + "!!!");
      }
    }
    std::cout << ObjexxFCL::format::I(
                     10, basic::options::option
                             [basic::options::OptionKeys::out::nstruct]()) +
                     " nb counts of random points match. Woot! "
              << tot << std::endl;
