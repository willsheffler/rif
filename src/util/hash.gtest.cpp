...
void test_map(Map* hp, int64_t MAXIDX, int64_t NSAMP) {
  ...
  cout << "rate " << NSAMP << " " << NROW << " " << MAXIDX << " "
       << t.elapsed_nano() / NSAMP / NROW
       << "ns, nonsense: " << (double)count / NSAMP << endl;
}
...
void fill_map(Map& h, int64_t MAXIDX, int64_t sparsity = 100ll) {
  ...
  cout << "done fill " << MAXIDX << " " << (double)h.size() / 1000000 << "M  "
       << (double)h.bucket_count() / 1000000000 *
              sizeof(typename Map::value_type)
       << "GB  " << (double)h.size() / h.bucket_count() << endl;
}
