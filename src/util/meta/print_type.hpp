...
struct PrintType {
  ...
  PrintType(std::ostream &_out = std::cout, std::string _indent = "",
            bool n = true)
  ...
  void operator()(m::pair<A, B> const &) const {
    out << indent << "mpl::pair< " << typeid(A).name() << ", "
        << typeid(B).name() << " > ";
    if (newline)
      out << std::endl;
    else
  ...
  void operator()(std::pair<A, B> const &) const {
    out << indent << "std::pair< " << typeid(A).name() << ", "
        << typeid(B).name() << " > ";
    if (newline)
      out << std::endl;
    else
  ...
  void operator()(f::pair<A, B> const &) const {
    out << indent << "fusion::pair< " << typeid(A).name() << ", "
        << typeid(B).name() << " > ";
    if (newline)
      out << std::endl;
    else
...
template <class T>
void print_type(std::ostream &out = std::cout, std::string indent = "",
                bool newline = true) {
...
struct PrintInstanceType {
  std::ostream &out;
  std::string indent;
  PrintInstanceType(std::ostream &_out = std::cout, std::string _indent = "")
...
struct PrintBFMapofVec {
  std::ostream &out;
  std::string indent;
  PrintBFMapofVec(std::ostream &_out = std::cout, std::string _indent = "")
      : out(_out), indent(_indent) {}
  template <typename T>
  void operator()(T const &x) const {
    out << indent << "KeyType: " << typeid(typename T::first_type).name()
        << std::endl;
