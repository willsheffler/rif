...
struct ActorConcept {
  ...
  ActorConcept(ActorConcept const &actor0, Position const &moveby) {
    data_ = actor0.data_;
  ...
  bool operator==(THIS const &o) const {
    return o.position_ == position_ && o.data_ == data_;
  }

  ///@brief necessary for testing only
  bool operator<(THIS const &o) const {
    return std::make_pair(position_, data_) <
           std::make_pair(o.position_, o.data_);
  }
