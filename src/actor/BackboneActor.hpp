...
struct BackboneActor {
  ...
  template <class V>
  BackboneActor(V n, V ca, V c, char aa = '-', char ss = '-', int i = 0)
  ...
  void from_n_ca_c(Vin _n, Vin _ca, Vin _c) {
    ...
    // V3 e1 = n - ca;
    V3 e1 =
        (c + n) / 2.0 - ca;  // from old motif stuff to maintain compatibility
    e1.normalize();
    V3 e3 = e1.cross(c - ca);
    e3.normalize();
    V3 e2 = e3.cross(e1);
    ...
    // V t = m * V(-0.865810,-1.764143,1.524857) + ca;// - (c+n)/2.0; // average
    // 'CEN' icoor
  ...
  // bool operator<(THIS const & o) const { return std::make_pair(position_,aa_)
  // < std::make_pair(o.position_,o.aa_); }
