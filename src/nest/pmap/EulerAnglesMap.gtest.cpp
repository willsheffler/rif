...
#include "nest/pmap/EulerAnglesMap.hpp"
...
TEST(EulerAnglesMap, covering) {
  ...
  for (int r = 1; r <= NRES; ++r) {
    ...
    double volfrac = (double)nest.size(r) / 2 * (maxdiff * maxdiff * maxdiff) *
                     4.0 / 3.0 * M_PI / 8.0 / M_PI / M_PI;
    double avgfrac = (double)nest.size(r) / 2 * (avgdiff * avgdiff * avgdiff) *
                     4.0 / 3.0 * M_PI / 8.0 / M_PI / M_PI;
    printf("%2i %16llu %10.5f %10.5f %10.5f %10.5f %10.5f\n", r,
           nest.size(r) / 2, maxdiff * 180.0 / M_PI, avgdiff * 180.0 / M_PI,
           maxdiff / avgdiff, volfrac, avgfrac);
    // cout << boost::format("%2i %20i %.7d %.7d") % r % nest.size(r) %
    // (maxdiff*180.0/M_PI) % volfrac << endl;
  }
}
