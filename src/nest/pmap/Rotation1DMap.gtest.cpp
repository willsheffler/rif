...
Vector3d get_axis(Matrix3d const &m) {
  AngleAxisd aa(m);
  if (aa.axis().dot(Vector3d(1, 1, 1)) < 0)
    return -aa.axis();
  else
    return aa.axis();
}
...
TEST(Rotation1DMap, flip_test) {
  {
    ...
    {
      Matrix3d m = nest.set_and_get(0, 0);
      ASSERT_DOUBLE_EQ(0.0, get_angle(m));
    }
    {
      Matrix3d m = nest.set_and_get(1, 0);
      ASSERT_DOUBLE_EQ(M_PI, get_angle(m));
      ASSERT_GE(get_axis(m).dot(flip), 0.9999);
    }
    {
      Matrix3d m = nest.set_and_get(0, 1);
      ASSERT_DOUBLE_EQ(-M_PI / 2.0, get_angle(m));
      ASSERT_GE(get_axis(m).dot(axis), 0.9999);
    }
    {
      Matrix3d m = nest.set_and_get(1, 1);
      ASSERT_DOUBLE_EQ(M_PI / 2.0, get_angle(m));
      ASSERT_GE(get_axis(m).dot(axis), 0.9999);
    }
    {
      ...
      ASSERT_DOUBLE_EQ(M_PI, get_angle(m));
      ASSERT_GE(get_axis(m).dot(Vector3d(0, sqrt(2) / 2, -sqrt(2) / 2)),
                0.9999);
    }
    {
...
TEST(Rotation1DMap, basic_test) {
  ...
  }
  {
    ...
    {
      Matrix3d m = nest.set_and_get(0, 1);
      ASSERT_DOUBLE_EQ(M_PI / 4.0, get_angle(m));
      ASSERT_GE(get_axis(m).dot(axis), 0.9999);
    }
    {
      Matrix3d m = nest.set_and_get(1, 1);
      ASSERT_DOUBLE_EQ(3 * M_PI / 4.0, get_angle(m));
      ASSERT_GE(get_axis(m).dot(axis), 0.9999);
    }
  }
}
...
TEST(Rotation1DMap, lookup_flip) {
  ...
  for (int i = 0; i < NITER; ++i) {
    ...
    nest.set_axis(axis);
    nest.set_flip_axis(
        axis.cross(Vector3d(gauss(rng), gauss(rng), gauss(rng))));
    nest.lb = uniform(rng) * 2 * M_PI - M_PI;
    nest.ub = nest.lb + uniform(rng) * (2 * M_PI - nest.lb - M_PI);
    BOOST_VERIFY(-M_PI <= nest.lb && nest.lb <= M_PI);
    BOOST_VERIFY(-M_PI <= nest.ub && nest.ub <= M_PI);
