...
TEST(TetracontoctachoronMap, covering) {
  ...
  cout << "resl              N     covrad     avgrad    cov/avg  overcover     "
          "avgcov  fracused"
       << endl;
  for (int r = 0; r <= NRES; ++r) {
    ...
    for (int i = 0; i <= NITER; ++i) {
      ...
      // cout << "index: " << index << " sample: " << q.coeffs().transpose() <<
      // endl;
    ...
    size_t count = 0;
    for (size_t i = 0; i < nest.size(r) / 24; ++i)
    ...
    printf("%2i %16lu %10.5f %10.5f %10.5f %10.5f %10.5f %7.5f\n", r, count,
           ...
           volfrac, avgfrac, (double)count / nest.size(r));
  }
}
...
TEST(TetracontoctachoronMap, nside_covering) {
  ...
  cout << "nside              N     covrad     avgrad    cov/avg  overcover    "
          " avgcov  fracused"
       << endl;
  for (int nside = 1; nside <= MAX_NSIDE; ++nside) {
    NEST<3, Matrix3d, TetracontoctachoronMap> nest(nside);
    ...
    size_t count = 0;
    for (size_t i = 0; i < nest.size(0) / 24; ++i)
      if (nest.set_state(i, 0)) ++count;
    count *= 24;
    double volfrac = (double)count * (maxdiff * maxdiff * maxdiff) * 4.0 / 3.0 *
                     M_PI / 8.0 / M_PI / M_PI;
    double avgfrac = (double)count * (avgdiff * avgdiff * avgdiff) * 4.0 / 3.0 *
    ...
    printf("%3i %16lu %10.5f %10.5f %10.5f %10.5f %10.5f %7.5f\n", nside, count,
           maxdiff * 180.0 / M_PI, avgdiff * 180.0 / M_PI, maxdiff / avgdiff,
           volfrac, avgfrac, (double)count / nest.size(0));
    std::cout.flush();
  }
}
...
// endl;
// 				// out << "MODEL" << std::endl;
// 				// io::dump_pdb_atom(out,  "Z"
// ,count2,Vector3d(0,0,0));
...
// nside              N     covrad     avgrad    cov/avg  overcover     avgcov
// fracused
//   1               24   62.76235   40.72950    1.54096    1.67355    0.45737
//   1.00000
//   2              192   39.38715   20.63925    1.90836    3.30899    0.47612
//   1.00000
//   3              648   26.80538   13.82587    1.93878    3.52024    0.48304
//   1.00000
//   4             1536   20.18428   10.38848    1.94295    3.56256    0.48571
//   1.00000
//   5             3000   16.18733    8.31389    1.94702    3.58904    0.48626
//   1.00000
//   6             5184   13.46439    6.92945    1.94307    3.56908    0.48651
//   1.00000
//   7             8232   11.58217    5.94196    1.94922    3.60750    0.48711
//   1.00000
//   8            12288   10.09158    5.19889    1.94111    3.56197    0.48702
//   1.00000
//   9            17496    9.00114    4.62175    1.94756    3.59884    0.48718
//   1.00000
//  10            24000    8.08815    4.16021    1.94417    3.58170    0.48740
//  1.00000
}
}
}
