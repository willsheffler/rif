...
void dumpatom(ostream &out, bool nothet, size_t iatom, string atom, string res,
              char chain, size_t ires, double x, double y, double z, double o,
              double b, string elem) {
  assert(atom.size() < 5);
  assert(res.size() < 4);
  ...
  char buf[128];
  snprintf(buf, 128,
           "%s%5lu %4s %3s %c%4lu    %8.3f%8.3f%8.3f%6.2f%6.2f %11s\n",
           nothet ? "ATOM  " : "HETATM", iatom, atom.c_str(), res.c_str(),
           chain, ires, x, y, z, o, b, elem.c_str());
  out << buf;
}

ostream &operator<<(ostream &out, Quaterniond const &q) {
  char buf[128];
  snprintf(buf, 128, "Quaterniond( %8.5f, %8.5f, %8.5f, %8.5f )", q.w(), q.x(),
           q.y(), q.z());
  out << buf;
  return out;
}
...
void dumpquats(string fname, vector<Quaterniond> &quats, Matrix4d proj,
               Quaterniond dir, double window = 0.1) {
  dir.normalize();
  ...
  BOOST_FOREACH (Quaterniond q, quats) {
    ...
    if (fabs(wxyz[1]) > window || fabs(wxyz[2]) > window ||
        fabs(wxyz[3]) > window)
      continue;
    char chain = 'A';
    if (q.x() >= q.w() && q.x() >= q.x() && q.x() >= q.y() && q.x() >= q.z())
      chain = 'B';
    if (q.y() >= q.w() && q.y() >= q.x() && q.y() >= q.y() && q.y() >= q.z())
      chain = 'C';
    if (q.z() >= q.w() && q.z() >= q.x() && q.z() >= q.y() && q.z() >= q.z())
      chain = 'D';
    dumpatom(out, true, ++ia, "QUAT", "QUT", chain, ++ir, 3000 * wxyz[1],
             3000 * wxyz[2], 3000 * wxyz[3], 1.0, 1.0, "C");
    ++count;
  }
...
void dumppoints(string fname, M const &pts) {
  ...
  for (int i = 0; i < pts.cols(); ++i) {
    // if( dir.dot(q) < mindot ) continue;
    dumpatom(out, true, ++ia, "PNT ", "PNT", 'A', ++ir, 100.0 * pts(0, i),
             100.0 * pts(1, i), 100.0 * pts(2, i), 1.0, 1.0, "C");
    ++count;
  }
  out.close();
}
...
vector<Quaterniond> make_half120cell() {
  ...
  // (0,0,0,±1)
  // by permuting coordinates. The remaining 96 vertices are obtained by taking
  ...
  vector<Quaterniond> half120cell;
  {
    ...
    for (int s1 = 1; s1 > -2; s1 -= 2) {
      ...
      for (int s2 = 1; s2 > -2; s2 -= 2) {
        for (int s3 = 1; s3 > -2; s3 -= 2) {
          ...
          c120.push_back(Quaterniond(-0.5, s1 * 0.5, s2 * 0.5, s3 * 0.5));
          // The remaining 96 vertices are obtained by taking even permutations
          // of
          // ½(±φ,±1,±1/φ,0).
          double const a = s1 * r / 2.0, b = s2 * 1 / 2.0, c = s3 / r / 2.0,
                       d = 0;
    ...
    Quaterniond alignq(-0.18683389537307968, -0.94619545649322123,
                       0.1868233140277023, -0.18682693324842686);
    Matrix4d alignment;
    alignment.fill(0);
    alignment.block(0, 0, 3, 3) = alignq.matrix();
    alignment(3, 3) = 1.0;
    BOOST_VERIFY(
        Vector4d(0, 0, 0, 1).isApprox(alignment * Vector4d(0, 0, 0, 1)));

    BOOST_FOREACH (Quaterniond q, c120) {
      q.coeffs() = alignment * q.coeffs();
      if (q.w() > 0 || (q.w() == 0 && q.x() > 0) ||
          (q.w() == 0 && q.x() == 0 && q.y() > 0) ||
...
Array<uint8_t, 60, 12> make_half120cell_neighbors(
...
TEST(hecatonicosachoron, static_inv_check) {
  ...
  for (int i = 0; i < 60; ++i) {
    ASSERT_TRUE(
        Quaterniond(1, 0, 0, 0)
            .isApprox(h120_cellcen<double>(i) * h120_cellceninv<double>(i)));
  }
}
...
TEST(hecatonicosachoron, cell_bounds) {
  ...
  // cout <<
  // (h120_cellcen<double>(0).coeffs()+cellnbr<double>(0,0).coeffs()).normalized().transpose()
  // << endl;
  // cout << std::setprecision(17) <<
  // (h120_cellcen<double>(0).coeffs()-cellnbr<double>(0,0).coeffs()).norm() <<
  // endl;
}
...
TEST(hecatonicosachoron, covering) {
  ...
  for (int r = 0; r <= NRES; ++r) {
    ...
    for (int i = 0; i <= NITER; ++i) {
      ...
      q.normalize();
      // cout << q << endl;
      Eigen::Quaterniond qcen(
          nest.set_and_get(nest.get_index(q.matrix(), r), r));
      maxdiff = std::max(maxdiff, q.angularDistance(qcen));
      avgdiff += q.angularDistance(qcen);
    }
    avgdiff /= NITER;
    size_t count = 0;
    for (size_t i = 0; i < nest.size(r) / 60; ++i)
