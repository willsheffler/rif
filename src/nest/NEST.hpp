...
struct NestBase {
  ...
  virtual bool virtual_get_state(Index index, Index resl,
                                 boost::any &result) = 0;
  ...
  virtual bool virtual_get_indices(boost::any const &val, Index resl,
                                   ...
                                   std::vector<Index> &indices_out) const = 0;
  ...
  virtual Index virtual_num_cells() const = 0;
...
typename boost::enable_if_c<
    has_const_member_fun_value_to_params<
        typename Nest::ParamMapType, bool, typename Nest::Value const &,
        typename Nest::Index, typename Nest::Params &,
        typename Nest::Index &>::value,
    typename Nest::Index>::type
wrap_virtual_get_index(Nest const &nest, boost::any const &val,
...
wrap_virtual_get_index(Nest const &nest, boost::any const &val,
                       typename Nest::Index resl) {
  std::cerr << "wrap_virtual_get_index: ParamMap type does not support lookups"
            << std::endl;
...
typename boost::enable_if_c<
    has_const_member_fun_value_to_params<
        ...
        typename Nest::Index &>::value,
    typename Nest::Index>::type
wrap_virtual_get_indices(Nest const &nest, boost::any const &val,
                         typename Nest::Index resl,
...
wrap_virtual_get_indices(Nest const &nest, boost::any const &val,
                         ...
                         std::vector<typename Nest::Index> &indices_out) {
  std::cerr
      << "wrap_virtual_get_indices: ParamMap type does not support lookups"
      << std::endl;
  util::meta::print_type<typename Nest::ParamMapType>();
  BOOST_VERIFY(false);
  std::exit(-1);
...
struct NEST :  // policy-based design, see Modern C++ Design
               public boost::mpl::if_c<_is_virtual, NestBase<_Index>,
                                       Empty>::type,  // conditional inherit
  ...
  typedef NEST<DIM, Value, ParamMap, StoragePolicy, Index, Float, is_virtual>
      ThisType;
  ...
  NEST(A const &a, B const &b, C const &c, D const &d)
      : ParamMapType(a, b, c, d) {}
  ...
  bool get_indicies(Value const &v, Index resl, Indices &indices_out,
                    Index &cell_index_out) const {
    assert(resl <= MAX_RESL_ONE_CELL);  // not rigerous check if Ncells > 1
    ...
    for (size_t i = 0; i < DIM; ++i) {
      // this min for bounds check kinda sucks, but it's necessary if you want
      // to allow points on the boundary
      indices_out[i] =
          std::min(((ONE << resl) - 1), static_cast<Index>(params[i] * scale));
      // std::cout << "NEST::get_indicies, indices_out " << indices_out[i] <<
      // std::endl;
    }
    return true;
  }
  ...
  void get_indicies_for_cell(Value const &v, Index resl, Index cell_index,
    ...
    Float scale = Float(ONE << resl);
    for (size_t i = 0; i < DIM; ++i) {
      // this crazy add/subtract avoids round towards 0 so params < 0 behave
      // correctly
  ...
  Index get_index(Indices const &indices, Index cell_index, Index resl) const {
    ...
    Index index = 0;
    for (size_t i = 0; i < DIM; ++i)
      index |= util::dilate<DIM>(indices[i]) << i;
  ...
  Index get_index(Value const &v, Index resl) const {
    ...
    Index cell_index;
    Indices indices;
    if (!get_indicies(v, resl, indices, cell_index))
  ...
  ///@brief put the zorder indices of all neighbors of bin at
  ///(indices,cell_index) for resolution resl into OutIter out
  template <class OutIter>
  void get_neighbors(Indices const &indices, Index cell_index, Index resl,
    ...
    functor = boost::bind(&ThisType::template push_index<OutIter>, this, _1,
                          cell_index, resl, out);
  ...
  bool get_state(Index index, Index resl, Value &v) const {
  ...
  virtual bool virtual_get_state(std::vector<Index> const &hindices,
    ...
    Params params;
    for (size_t i = 0; i < DIM; ++i)
      params[i] = (static_cast<Float>(hindices[iindex + i]) + 0.5) * scale;
    iindex += DIM;
    Value &v(*boost::any_cast<Value *>(result));
  ...
  virtual bool virtual_get_indices(boost::any const &val, Index resl,
    ...
    return impl::wrap_virtual_get_indices(*this, val, resl, cell_index_out,
                                          indices_out);
  }
};
...
template <int DIM, class Value,
          ...
          template <class> class StoragePolicy, class Index, class Float,
          bool is_virtual>
std::ostream &operator<<(std::ostream &out,
                         NEST<DIM, Value, ParamMap, StoragePolicy, Index, Float,
