...
TEST(geom_4d, tetracontoctachoron_cell_lookup) {
  ...
  Map<Matrix<Float, 48, 4, RowMajor> const> t24(
      numeric::get_raw_48cell<Float>());
  ...
  for (int i = 0; i < NITER; ++i) {
    ...
    // cell2[i] = hyperface_dist==mx ? facecell : (corner_dist==mx ?
    // cornercell+8 : edgecell+24);
  }
  cout << "bt24 clever rate: " << (double)NITER / clever.elapsed_nano() << endl;

  for (int i = 0; i < NITER; ++i) {
    if (cell[i] != cell2[i]) {
      ASSERT_FLOAT_EQ(t24.row(cell[i]).dot(samp[i]),
...
TEST(geom_4d, tetracontoctachoron_half_cell_lookup) {
  ...
  Map<Matrix<Float, 24, 4, RowMajor> const> t24h(
      numeric::get_raw_48cell_half<Float>());
  ...
  cout << "hbt24 clever rate: " << (double)NITER / clever.elapsed_nano()
       << endl;
  ...
  for (int i = 0; i < NITER; ++i) {
    ...
    if (cell[i] != cell2[i]) {
      ASSERT_FLOAT_EQ(t24h.row(cell[i]).dot(samp[i]),
