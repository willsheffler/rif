...
#include "search/SpatialBandB.hpp"
...
struct TestScoreBBactor {
  ...
  TestScoreBBactor()
      : target_inv_(Xform::Identity()), radius_(1.0), lever_(10.0) {}
  ...
  Result operator()(BBActor const &a, Config const &c) const {
    ...
    // }
    // cout << a.position().rotation().trace() << " " <<
    // target_inv_.rotation().trace() << endl;
    ...
    // cout << roterr << " " << ( ( trace - 1.0 ) / 2.0 ) << " " << acos(( (
    // trace - 1.0 ) / 2.0 ) )*180.0/3.14159 << endl;
...
TEST(SpatialBandB, test_test_utils) {
  ...
  typedef objective::ObjectiveFunction<boost::mpl::vector<TestScoreBBactor>,
                                       int>
      Objective;
